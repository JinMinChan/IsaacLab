...
class CameraFrankaEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: CameraFrankaEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # 이미지 저장 경로 설정
        self.yolo_model = YOLO('/home/vision/Downloads/minchan_yolo/train/weights/best.pt', verbose=False)
    ...
    def detect_valve_with_yolo(self, env_id: int):
        ...
        if self.camera.data.output["rgb"] is not None:
            ...
            # YOLO 예측
            results = self.yolo_model.predict(bgr_image, verbose=False)
    ...
    def _compute_rewards(
        ...
        for env_id in range(num_envs):
            ...
            valve_detected.append(detected)

        # 보상 계산
        reward = torch.tensor(
            [10.0 if detected else 0.0 for detected in valve_detected], device=self.device
        )
