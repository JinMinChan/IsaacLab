...
class CameraFrankaEnv(DirectRLEnv):
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # robot_dof_targets 및 valve_detected_results 초기화
        self.prev_robot_dof_targets[env_ids, :2] = 0.0
    ...
    def _compute_rewards(
        ...
        yolo_reward = torch.tensor(
            [5.0 if detected else 0.0 for detected in valve_detected], device=self.device
        )
        # ** Mobile 보상 계산 (탐지 시 활성화) **  
        ...
        # 탐지 여부에 따라 보상 활성화
        mobile_distance_reward = mobile_distance_reward * valve_detected.float()

        # 총 보상 계산
        rewards = (
            + yolo_reward  # YOLO 탐지 보상
            + mobile_distance_reward  # Mobile 보상
            - franka_action_penalty  # 행동 패널티
        )

        # 로그 정보 업데이트
        self.extras["log"] = {
            "total_rewards": rewards.mean().item(),  # float로 변환 후 저장
            "yolo_reward": yolo_reward.mean().item(),
            "mobile_distance_reward": mobile_distance_reward.mean().item(),
            "action_penalty": -franka_action_penalty.mean().item(),
            "mobile_dist": mobile_dist.mean(),  # Mobile 거리 로깅
        }
