...
class FrankaValveEnv(DirectRLEnv):
    ...
    def _compute_rewards(
        ...
        xy_distance = torch.norm(
            franka_grasp_pos[:, :2] - valve_grasp_pos[:, :2], p=2, dim=-1
        )

        # XY 평면 정렬 보상
        xy_alignment_reward = 10.0 * torch.exp(-3.0 * xy_distance)

        # XY 평면 정렬 패널티 (거리가 너무 큰 경우)
        ...
        # ** Step 2: Z 축 접근 단계 (XY 정렬 기준 충족 시) **
        is_xy_aligned = xy_distance < 0.05  # XY 정렬 기준 충족 여부
        z_diff = torch.abs(franka_grasp_pos[:, 2] - valve_grasp_pos[:, 2])

        # Z 축 보상 (XY 정렬이 충족된 경우만 계산)
        z_reward = torch.where(
            is_xy_aligned,
            5.0 * torch.exp(-10.0 * z_diff),
            torch.tensor(0.0, device=self.device),
        )

        # Z 축 패널티 (XY 정렬 기준 충족 후 Z 축 벗어난 경우)
        ...
        # ** Step 3: 최종 도달 보상 (XY + Z 정렬 기준 모두 충족 시) **
        ...
        rewards = (
            ...
            + rot_reward  # 축 정렬 보상
            + finger_reward_open  # 손가락 벌림 보상
            + finger_reward_close  # 손가락 간 거리 보상
            - action_penalty  # 행동 패널티
        )

        # ** 로깅 정보 업데이트 **
        self.extras["log"] = {
            "xy_distance": xy_distance.mean(),
            "z_diff": z_diff.mean(),
            "xy_alignment_reward": xy_alignment_reward.mean(),
            "xy_alignment_penalty": xy_alignment_penalty.mean(),
